...
export class SideNavExpandedComponent implements OnInit, OnDestroy {
  ...
  private computeGroups() {
    const groupDefs: { key: string; title: string; match: (title: string) => boolean }[] = [
      { key: 'plan', title: 'Plan & Design', match: (t) => ['Define', 'Design', 'Documentation'].includes(t) },
      ...
      { key: 'govsec', title: 'Governance & Security', match: (t) => ['Compliance', 'Guard Rails', 'Audit Trails'].includes(t) },
      { key: 'org', title: 'Organization', match: (t) => ['User Management', 'Billing', 'Quotations'].includes(t) },
